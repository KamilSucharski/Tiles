...
public final class TilesView extends LinearLayout {
    ...
    public void configure(final TilesViewConfiguration configuration) {
        this.configuration = configuration;
    }

    @RequiresApi(Build.VERSION_CODES.O)
    public void display(final Map<java.time.LocalDate, Integer> highlightedDays) {
        final Map<LocalDate, Integer> highlightedDaysCompat = new HashMap<>();
        for (final Map.Entry<java.time.LocalDate, Integer> entry : highlightedDays.entrySet()) {
            final Date date = Date.from(entry
                .getKey()
                .atStartOfDay()
            ...
            highlightedDaysCompat.put(LocalDate.fromDateFields(date), entry.getValue());
        }

        displayCompat(highlightedDaysCompat);
    }

    public void displayCompat(final Map<LocalDate, Integer> highlightedDays) {
    ...
    private LinearLayout createRow(final int rowNumber,
                                   ...
                                   final Map<LocalDate, Integer> highlightedDays) {
    ...
    private View createDayView(final Context context,
                               final Map<LocalDate, Integer> highlightedDays,
                               final int day,
                               final int month,
                               final int year,
    ...
    private void setCorrectBackgroundOnView(final View view,
                                            final Map<LocalDate, Integer> highlightedDays,
                                            final int day,
                                            final int month,
                                            final int year) {
